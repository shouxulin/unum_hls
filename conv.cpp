...
void u2f(unum_s *u,gnum_s* f){
    ...
    ap_uint<1> hidden = (u->range(utagsize+ut.esize-1,utagsize).or_reduce());
...
void f2u(gnum_s *gnum, unum_s* unum){
    ...
    if (fs <= fsizemax){
        ...
        unum->range(utagsize+fs-1,utagsize) = (gnum->f>0?(gnum->f.range(fractionsize-1,fractionsize-fs)):((-gnum->f).range(fractionsize-1,fractionsize-fs)));
    }
    ...
    for (int es = 1; es <= esizemax; ++es) {
        ...
        bias = (1<<(es-1))-1;
        unum->range(utagsize+fs+es-1, utagsize+fs) = gnum->e+bias;
